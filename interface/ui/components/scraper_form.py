...
from interface.utils.start_scraper import start_workday_task
...
class ScraperForm:
    ...
    def _render(self):
        ...
        # Platform link input
        platform_link = ui.input(
            label='Platform Link',
            placeholder='https://...'
        ).classes('w-full').props('outlined')
        
        # Job server ID input
        jobserver_id = ui.input(
            label='Job Server ID',
            placeholder='Enter job server ID'
        ).classes('w-full').props('outlined')
        ...
        def start_scraper():
            ...
            try: 
                is_started = start_workday_task(self.db, save_to_db.value, job_id, platform_link.value, name, is_test.value)                    
                if is_started: 
                    with message_container:
                    ...
                    ui.timer(1.0, lambda: ui.navigate.to('/workday-scraper'), once=True)
                else:
                    with message_container:
                        ui.notify('Failed to start scraper: Process ID is null', type='negative')
                        
            except Exception as e:
                with message_container:
                    ui.notify(f'Error starting scraper: {str(e)}', type='negative')
        
        ui.button('Start Scraper', on_click=start_scraper, icon='play_arrow').props('color=primary').classes('mt-4')
