...
class ProgressDisplay:
        ...
        with ui.card().classes('w-full p-4'):
            ...
            with ui.row().classes('w-full gap-8 mb-4'):
                with ui.column():
                    ...
                    ui.label(str(data.platform)).classes('metric-value text-sm font-bold')
            ...
            # Expandable details
            with ui.expansion('View Details', icon='info').classes('w-full'):
                # Progress bar
                ...
                with ui.row().classes('w-full gap-8 mb-4'):
                    with ui.column():
                        ...
                        ui.label(f'{data.current}/{data.total}').classes('metric-value text-sm')
                ...
                ui.label(f'Last updated: {data.last_updated}').classes('caption text-xs')
    ...
    def _refresh_process(self, data):
        ...
        try:
            is_started = start_workday_task(self.db, jobserver_id=data.id, platform_link=data.platform_url, name=data.platform, save_to_db=True, is_test=False)
            if is_started:
